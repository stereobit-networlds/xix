...
$__EVENTS['SHKATEGORIES_DPC'][5]='tree_navigation';
$__EVENTS['SHKATEGORIES_DPC'][6]='tree_past_navigation';
...
$__ACTIONS['SHKATEGORIES_DPC'][5]='tree_navigation';
$__ACTIONS['SHKATEGORIES_DPC'][6]='tree_past_navigation';
...
class shkategories {
	...
	var $dir_prefix, $rootdomain, $default_app_name;
	...
	function event($event=null) {
	    ...
	    switch ($event) {
		  ...
		  case 'tree_past_navigation': die($this->tree_past_navigation());break;
		  case 'tree_navigation': die($this->tree_navigation());break;
          case 'openf' : 
		                        break;			
          case 'shkategories' :
	...
	function action($action=null) {	
	    ...
	    switch ($action) {
		  ...
		  case 'tree_past_navigation': break;
          case 'tree_navigation': break;			
          case 'openf'        : 
		                        break;		
          case 'shkategories' :
	...
	protected function get_image_icon($catcode=null, $retimage=false) {	
        $alsoseedir = $this->urlpath . $this->inpath . '/' . $this->showcatimagepath;
    ...
    function view_category($ddir,$type=1,$mode=0,$group=null,$cmd=null,$tokens=null,$setofgroups=null,$setofdepths=null,$template=null) {
        ...
        if ($ddir)  {
		   ...
		   $t = ($isajax) ? GetReq('t') : ($cmd?$cmd:'shkategories');
		   //$ti come into loop
           ...
           foreach ($ddir as $line_num => $line) {	
                 ...
                 if ($this->usetablelocales) {
                  ...
                  switch ($type) {
                    default : 
                    case  1 :  if ($tokens) {
					             $purl = seturl("t=$ti&cat=$gr",null,null,null,null,$this->rewrite);
								 
					             $mytokens[] = ($isajax) ? seturl('#',$loctitle,null,"onClick=\"ajaxcall('xixcp_div', '$purl');\"",null,$this->rewrite) . " " . $this->outpoint . " ":
								                           seturl("t=$ti&cat=$gr",$loctitle,null,null,null,$this->rewrite) . " " . $this->outpoint . " ";
								 $tok2[$i][] = ($isajax) ? seturl('#',$loctitle,null,"onClick=\"ajaxcall('xixcp_div', '$purl');\"",null,$this->rewrite) . " " . $this->outpoint . " " :
								                           seturl("t=$ti&cat=$gr",$loctitle,null,null,null,$this->rewrite) . " " . $this->outpoint . " ";
								 $tok2[$i][] = $gr; //cat name
								 ...
								 $tok2[$i][] = $this->get_image_icon($gr, true); //just the url
								 $tok2[$i][] = rand();
							   }	 
					           else {
							     if ($isajax) {
								    $purl = seturl("t=$ti&cat=$gr",null,null,null,null,$this->rewrite);
									$to_be_print .= seturl('#',$loctitle,null,"onClick=\"ajaxcall('xixcp_div', '$purl');\"",null,$this->rewrite) . " " . $this->outpoint . " ";
								 }
								 else
	...
	function read_selected_tree($group,$cmd=null,$stylesheet=null,$outpoint=null,$br=1,$debug=null,$nounion=false) {
	    $t = $cmd?$cmd:'klist';
	    //$mystylesheet = $stylesheet?$stylesheet:'group_category_title';		
	  	
	    $ddir = $this->read_tree($group,$debug,1,$nounion);		
	...
	function show_selected_branch($id,$line,$t=null,$myselcat=null,$expand=null,$stylesheet=null,$outpoint=null,$br=1,$template=null,$linkclass=null,$nounion=false) {
	       $mystylesheet = $stylesheet?$stylesheet:'group_category_title';	
		   //echo '>',$outpoint;
           if ($template) { //template
		    ...
		    if ($line) {	
			  ...
			  $winbody = $this->read_selected_tree($folder,$t,$mystylesheet,$outpoint,null,null,$nounion);
 		      $data[] = $winbody;
              $attr[] = "left;";	
    ...
    function show_selected_tree($cmd=null,$group=null,$showroot=null,$expand=null,$viewlevel=null,$stylesheet=null,$outpoint=null,$br=1,$template=null,$linkclass=null,$nounion=false) {
	  $mystylesheet = $stylesheet?$stylesheet:'group_category_title';	
	  $cat = rawurldecode(str_replace('_',' ',GetReq('cat')));	
	  ...
	  if ($myselcat) 	
	    $ddir = $this->read_tree($myselcat,null,1,$nounion);	
	  elseif ($showroot) 
	    $ddir = $this->read_tree(null,null,1,$nounion);
				
	  $i=0;	 
      if ($ddir)  {	   
          reset($ddir);
          foreach ($ddir as $id => $line) {		  
            $out .= $this->show_selected_branch($id,$line,$t,$myselcat,$expand,$mystylesheet,$outpoint,$br,$template,$linkclass,$nounion);
			$i+=1; 
		  }//foreach				
	  }//if ddir
	...
	function show_submenu($cmd=null,$viewtype=3,$group=null,$notheme=null, $rendertable=false, $nounion=false) {
		$group = $group?$group:str_replace('_',' ',GetReq('cat'));	
	
		$result = $this->read_tree($group,null,null,$nounion);	
    ...
    function read_union_tree($g=null, $orderctg=null, $nounion=false) {
		$db = GetGlobal('db');	
		$lan = getlocal();
		$mylan = $lan?$lan:'0';
	    ...
	    if (strlen(trim($g))>0) {
   		...
   		if ($nounion) {//only root app categories
	    ...
	    foreach ($appresult as $a=>$app) {
			...
			if ($this->usetablelocales) {		     
			  ...
			  switch ($depth) {
				case 1 : $sSQL = "(select cat3,cat{$f}3,cat2,cat{$f}2 from {$appname}.categories where "; break;
				case 2 : $sSQL = "(select cat4,cat{$f}4,cat3,cat{$f}3,cat2,cat{$f}2 from {$appname}.categories where "; break;
				case 3 : $sSQL = "(select cat5,cat{$f}5,cat4,cat{$f}4,cat3,cat{$f}3,cat2,cat{$f}2 from {$appname}.categories where "; break;
				case 4 : $sSQL = "(select null from {$appname}.categories"; break;
				default: $sSQL = "(select cat2,cat{$f}2 from {$appname}.categories where "; break;
			  }
			  ...
			  switch ($depth) {
				...
				case 1 : $sSQL .= "(cat2='" . str_replace('_',' ',$group[0]) . "' or cat{$f}2='" . str_replace('_',' ',$group[0]) . "') and "; 
				default: $sSQL .= "ctgid>0 and active>0 and view>0 order by ctgoutlnorder)";

			  }	   
			}
			else {
			  switch ($depth) {
				case 1 : $sSQL = "(select cat3,cat2 from {$appname}.categories where "; break;
				case 2 : $sSQL = "(select cat4,cat3,3,cat2 from {$appname}.categories where "; break;
				case 3 : $sSQL = "(select cat5,cat4,cat3,cat2 from {$appname}.categories where "; break;
				case 4 : $sSQL = "(select null from {$appname}.categories"; break;
				default: $sSQL = "(select cat2 from {$appname}.categories where "; break;
			  }
			  ...
			  switch ($depth) {
				...
				case 1 : $sSQL .= "cat2='" . str_replace('_',' ',$group[0]) . "' and "; 
				default: $sSQL .= "ctgid>0 and active>0 and view>0 order by ctgoutlnorder)";
			  }	
			}
			//echo $sSQL;
	...
	//read tree table
	function read_tree($g=null,$debug=null,$orderctg=null, $nounion=false) {
        $db = GetGlobal('db');	   
		//echo $nounion; 	
		$sSQL = $this->read_union_tree($g,$orderctg,$nounion);		 
	    //echo $sSQL;   
	   
        //cache queries 
	    //if ($this->cachequeries) 
	      //$result = $db->CacheExecute($this->cachetime,$sSQL);
        //else 
    ...
    function union_analyzedir($splitx=null, $depth=0) {
	    $db = GetGlobal('db');
		$lan = getlocal();
	    $f = $lan?$lan:'0';		
	    ...
	    foreach ($appresult as $a=>$app) {	
		    ...
		    $sSQL = "(select distinct cat2,cat{$f}2,cat3,cat{$f}3,cat4,cat{$f}4,cat5,cat{$f}5 from {$appname}.categories where ";
    ...
    function analyzedir($group,$startup=0,$force_analyze=false) {

	    //if executed at event...
		if ((!$force_analyze) && ($this->cat_result))
        ...
        if ($this->usetablelocales)	{
		    ...
		    $sSQL = $this->union_analyzedir($splitx, $depth);
			//echo $sSQL;
    ...
    function view_analyzedir($cmd=null,$prefix=null,$startup=0,$symbol=null,$tokens=null,$scat=null,$ajax=null) { 	
		//$t = GetReq('t');//$this->dirview; //GetReq('t'); 
		$t = ($cmd ? (($ajax) ? 'ajax_'.$cmd : $cmd) : GetReq('t'));	
		$g = $scat ? str_replace(' ','_',$scat) : str_replace(' ','_',GetReq('cat'));
		$a = GetReq('a');	
		...
		//analyze dir	
        //echo '---------',$g,'->';		
        $adirs = $this->analyzedir($g,$startup,$scat);	//if scat arg force analyze				
		//echo implode('->',$adirs),'<br/>';
		    ...
		    foreach ($adirs as $id=>$cname) {	
    		  ...
    		  if ($m2<=$max) { //< .......... link last element 
                ...
                if ($cname != $this->home) {
			      ...
			      $thisurl = seturl("t=$cmd&cat=".GetReq('cat'),null,null,null,null,$this->rewrite);
                  $purl = seturl("t=$t&cat=$mygroup&page=".$page,null,null,null,null,$this->rewrite);			  
				  $aprint .= ($ajax) ? "<A href=\"$thisurl#\" onClick='ajaxcall(\"{$ajax}\",\"".$purl."\")'>" :
			                           "<A href=\"" . $purl . "\">";				   
		        }	
	            else {
   	              $aprint .= "<A href=\"" . seturl("t=",null,null,null,null,$this->rewrite) . "\">";
				  $purl = seturl("t=",null,null,null,null,$this->rewrite);
				}  
		         ...
		         if ($tokens) {
				  ...
				  $mytokens[] =  ($ajax) ? seturl('#',$locname,null,"onClick=\"ajaxcall('{$ajax}', '$purl');\"",null,$this->rewrite) :
				                           $aprint . $locname . "</A>"; 
				  $mytokens[] = $dmark;				   
			     }
			     else {			  
    ...
    function tree_navigation($cmd=null,$prefix=null,$home=0,$scat=null,$enable_ajax=false, $ajax=false) {
		
	    //call from javascript ajax scroll
	    if (($enable_ajax) && (!GetReq('ajax'))) {
	        
			$out = GetGlobal('controller')->calldpc_method('fronthtmlpage.get_scrollid use '.get_class($this).'+tree_navigation+'."cmd|prefix|home|cat+$cmd|$prefix|$home|".GetReq('cat')); 
			return ($out);
		}
		...
		//when skin return 2 brs	
		if (!$scat) { //only when called with no cat arg
		    //save past navs
			$pastnav = (array) GetSessionParam('pastnav') ?	
						unserialize(GetSessionParam('pastnav')) :
						array(); 

			$pastnav[GetReq('cat')] = GetReq('cat');
			//print_r($pastnav);
			SetSessionParam('pastnav',serialize($pastnav));				
		
			$exit = (defined('VON_DPC')) ? 
				    (GetSessionParam('skin')!='off'? true: false) : 
				     false;
		    if ($exit) 
				return ('<br/><br/>');		   
		}
	    ...
	    $template = (defined('VON_DPC')) ? 
				   (GetSessionParam('skin')!='off'? 'fpkatnav-'.GetSessionParam('skin').'.htm': 'fpkatnav.htm') : 
				   'fpkatnav.htm';	
	    ...
	    if (($template) && is_readable($t)) {
          ...
          //directory path	
          $navdata = $this->view_analyzedir($cmd,$prefix,$home,null,1,$scat,$ajax);
		  //print_r($navdata);
	...
	//load past nav paths
	public function tree_past_navigation($cmd=null,$prefix=null,$home=0,$ajax=false,$enable_ajax=false) {
	    //call from javascript ajax scroll
	    if (($enable_ajax) && (!GetReq('ajax'))) {

			$out = GetGlobal('controller')->calldpc_method('fronthtmlpage.get_scrollid use '.get_class($this).'+tree_past_navigation+'."cmd|prefix|home|cat|ajax+$cmd|$prefix|$home|".GetReq('cat')."|$ajax"); //save cat to analyze		
			return ($out);
		}
		//get GET func params when call from scroll
		foreach ($_GET as $gname=>$gvalue) 
		...
		$pastnav = unserialize(GetSessionParam('pastnav'));
		//print_r($pastnav);		
        if (!empty($pastnav)) {
		
			foreach ($pastnav as $i=>$cat) {
				$ret .= $this->tree_navigation($cmd,$prefix,$home,$cat,null,$ajax);
			}
			return ($ret);
		}	
	...
	function union_search_tree($i=0,$text2find=null) {
	    $db = GetGlobal('db');
		$lan = getlocal();   
	    $mylan = $lan?$lan:'0';
		$f = ($this->usetablelocales) ? $mylan : null;
	    ...
	    foreach ($appresult as $a=>$app) {	
			...
			$sSQL.= " from {$appname}.categories where ";
	...
	function search_tree($text2find=null,$cmd='shkategories',$template=null) {
	   ...
	   for($i=2;$i<=5;$i++) {//echo $i,'<br>';
         ...
         $sSQL = $this->union_search_tree($i,$text2find);
	...
	function asksql($cat,$presel=null) {
	    ...
	    foreach ($appresult as $a=>$app) {	
			...
			if ($this->usetablelocales) {		     
				...
				$sSQL = "(select $cat,$mylancat from {$appname}.categories where ctgid>0 and active>0 and view>0 and search>0";
			}  
			else {
				$f = null;
				$sSQL = "(select $cat from {$appname}.categories where ctgid>0 and active>0 and view>0 and search>0";	   
			}  
