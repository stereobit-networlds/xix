...
$b = GetGlobal('controller')->require_dpc('google/Google_Client.lib.php');
require_once($b);
require_once 'cp/dpc/google/contrib/Google_Oauth2Service.php';

//$c = GetGlobal('controller')->require_dpc('twitter/tmhOAuth.lib.php');
//require_once($c);
//require_once 'cp/dpc/twitter/tmhUtilities.php';
...
$__LOCALE['SHLOGIN_DPC'][126]='_glogin;Login;Εισαγωγή';
$__LOCALE['SHLOGIN_DPC'][127]='_tlogin;Login;Εισαγωγή';
$__LOCALE['SHLOGIN_DPC'][128]='_gloginlabel;Login with Google;Εισαγωγή με το Google';
...
class shlogin {
	...
	function shlogin() {
	   ...
	   //facebook login 
	   $this->facebook_id = '1402380970015617';
	   $this->facebook_key = '8e09f14a39c54591921127ef2697cdd0';
    ...
    function event($sAction) {
       ...
       if (!$this->msg) {
         ...
         switch($sAction)   {
            ...
            case 'shlogin'     : $this->login_javascript();  break;

            case "dologin"     :  $this->login_successfull = ($_REQUEST['oauth_verifier']/* || $_REQUEST['authenticate']*/) ? 
			                                                 $this->do_twitter_login() :
															  ((isset($_GET['code'])) ?  
			                                                    $this->do_google_login() :
			                                                     (($this->is_fb_logged_in()) ? 
            ...
            case "dologout":     if (defined('UONLINE_DPC'))
    	                            GetGlobal('controller')->calldpc_method('uonline.isOffline');
								
                                 if ($this->is_twitter_logged_in()) {	
									$this->do_twitter_logout();
                                 }								 
                                 elseif ($this->is_google_logged_in()) {
									$this->do_google_logout();
								 }
								 elseif ($this->is_fb_logged_in()) {
...
FBLOGIN;
	...
	function facebook_login($init_only=false, $js=false) {
	...
	function is_fb_logged_in() {
	    //print_r($_COOKIE);
		
		$this->facebook = new Facebook(array(
							'appId'  => '1402380970015617',
							'secret' => '8e09f14a39c54591921127ef2697cdd0',
							));		
	...
	//G
	function google_login($buttonlabel=null, $sizestyle=null) {
	    $bt = $buttonlabel ? $buttonlabel :
		      localize('_glogin',getlocal());
		$bs = $sizestyle ? $sizestyle : 'btn-large';	  
	
		if ($this->is_google_logged_in()) return('Google logged in');	
		...
		//$this->gclient->setScopes(array('https://www.googleapis.com/auth/userinfo.email','https://www.googleapis.com/auth/userinfo.profile'));
		$authUrl = $this->gclient->createAuthUrl();
		
		if ($authUrl)	
			//$ret = "<a class=\"login\" href=\"$authUrl\">Google Account Login</a>";
			$ret = "<a class='btn $bs' href='$authUrl'>" . $bt . "</a>";
			//$ret = $this->displayGLoginButton();
		else
            $ret = "No auth Url";//"<a class="logout" href="?logout">Logout</a>";		
			
		return ($ret);
	}
	
	function displayGLoginButton() {
		return '<span id="signinButton"><span class="g-signin" data-callback="signinCallback" data-clientid="111972355706-5i4gdd3d8ci401rp2umro3itonorg6tq.apps.googleusercontent.com" data-cookiepolicy="single_host_origin" data-requestvisibleactions="http://schemas.google.com/AddActivity" data-scope="https://www.googleapis.com/auth/plus.login"></span></span>';
	}	
	
	function is_google_logged_in($init=false) {
	    //print_r($_COOKIE);
		$this->gclient = new Google_Client();
		$this->gclient->setClientId('111972355706-5i4gdd3d8ci401rp2umro3itonorg6tq.apps.googleusercontent.com');
		$this->gclient->setClientSecret('axlYm5fpYspb3TW_zckzNNtd');
		$this->gclient->setRedirectUri('http://www.xix.gr/oauth2callback');
		$this->gclient->setDeveloperKey('AIzaSyDgNfkKVLswLHyY4tpCT7oJQjIBDVLpoYs');	
						
		if  (!is_object($this->gclient)) 
			return false;
			
		$this->goauth2 = new Google_Oauth2Service($this->gclient);	

        $this->gclient->setApplicationName("Google login"); 
		$this->gclient->setRedirectUri('http://www.xix.gr/xix.php?t=dologin');//http://localhost/admin/index.php?m=1&e=calendar');		
			
	    if (($init===true) && (isset($_GET['code']))) {
		
			$this->gclient->authenticate();
			
			$gtoken = $this->gclient->getAccessToken();
			//$_SESSION['gtoken'] = $gtoken;
			SetSessionParam('gtoken',$gtoken);
			//header('Location: http://' . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF']);
			//return ($gtoken);//true; //session['gtoken'] has not yet saved
			$this->gclient->setAccessToken($gtoken);
		}
		
		//if ($_SESSION['gtoken']) {
		if ($gtoken = GetSessionParam('gtoken')) {
			$this->gclient->setAccessToken($gtoken);//$_SESSION['gtoken']);
			return true;
		}	
	...
	function twitter_login() {
		...
		if ($this->is_twitter_logged_in()) return('Twitter logged in');	
						
		$authUrl = $this->do_twitter_login(true);
		
		if ($authUrl)	
			$ret = "<a class='btn btn-large' href='$authUrl'>" . localize('_tlogin',getlocal()) . "</a>";
		else
            $ret = "No auth Url";//"<a class="logout" href="?logout">Logout</a>";		
	...
	function displayTLoginButton() {
	    return false; //<<DISABLED
		
	    $authUrl = $this->do_twitter_login(true);
		return seturl($authUrl ,'Twitter');
	}		
	
	function is_twitter_logged_in() {
	    return false; //<<DISABLED
...
/*
    ...
    function quickform($attr=null,$after_goto=null,$dpc_after_goto=null,$param_name=null,$param=null) {
       ...
       if ($tokensout) {
         ...
         $tokens[] = "<input type=\"hidden\" name=\"FormAction\" value=\"dologin\">" .
					 "<input type=\"hidden\" name=\"FormName\" value=\"Login\">" .
		             "</form>" . 
					 $this->facebook_login(false,true) .
					 $this->google_login(localize('_gloginlabel',getlocal()),'btn-small');
	...
	function do_twitter_login($init=false) {
	    $db = GetGlobal('db');	
		return false; //<<<DISABLED
		...
		'consumer_key'    => 'F69llocWWpoD8JxlEVv2sg',
		'consumer_secret' => 'k69p9GyXTGaVKNe7ovWCt7LKOKGeG1rKWGbBDuL09Y',
		));	
		...
		if (isset($_REQUEST['oauth_verifier'])) {
			...
			if ($code == 200) {
				...
				header("Location: 'http://www.xix.gr/xix.php'");
				//second step verify credencials
				//$callback = 'http://www.xix.gr/xix.php?t=dologin&authenticate=1';
				//header("Location: '{$callback}'");				
				
				//$resp = json_decode($this->tmhOAuth->response['response']);
				//print_r($resp);
				//echo $resp->screen_name.'@twitter.com';
				$email = $this->tmhOAuth->config['user_token'].'@twitter.com';
				
				$sUsername = $email; //???? not returnable
				$sName = $email;
				$sId = $email;
				...
				if (defined('SHUSERS_DPC')) {
					...
					$uret = GetGlobal('controller')->calldpc_method('shusers.username_exist use '.$email);
			
					if (!$uret) {//insert twitter user			
						$sSQL = "insert into users (code2,fname,lname,email,notes,username,subscribe,seclevid) values ";
						$sSQL.= "('{$sUsername}','{$sName}','{$userInfo['given_name']}','{$email}','TWITTER','{$email}',1,1)";
						$ret = $db->Execute($sSQL);
					}
					else {
						$sSQL = "UPDATE users set notes='TWITTER',fname='{$sName}',lname='{$userInfo['given_name']}' WHERE username='{$email}'";
						$ret = $db->Execute($sSQL);
						//$uret = true;				
					} 			  
		...
		elseif ( isset($_REQUEST['authenticate']) || isset($_REQUEST['authorize']) ) {
			$callback = isset($_REQUEST['oob']) ? 'oob' : 'xix.php';//$here;
		...
		//elseif ( isset($_REQUEST['authenticate']) || isset($_REQUEST['authorize']) ) {
			...
			$code = $tmhOAuth->request('GET', $this->tmhOAuth->url('1/account/verify_credentials'));
			if ($code == 200) {
				$resp = json_decode($this->tmhOAuth->response['response']);
...
*/	
	function do_google_login() {
		...
		if ($this->is_google_logged_in(true)) {
		
            //$oauth2 = new Google_Oauth2Service($this->gclient);
			$userInfo = $this->goauth2->userinfo->get();//$me = $plus->people->get('{{ID}}');
			...
			//$_SESSION['gtoken'] = $this->gclient->getAccessToken();
			$gtoken = $this->gclient->getAccessToken();
			SetSessionParam('gtoken',$gtoken);
			
			$sUsername = $email;
			$sName = $userInfo['name'] ? $userInfo['name'] : $email;
			$sId = $email;
			...
			if (defined('SHUSERS_DPC')) {
              ...
              if (!$uret) {//insert google user			
				$sSQL = "insert into users (code2,fname,lname,email,notes,username,subscribe,seclevid) values ";
				$sSQL.= "('{$sUsername}','{$sName}','{$userInfo['given_name']}','{$email}','GOOGLE','{$email}',1,1)";
				$ret = $db->Execute($sSQL);
			  }
              else {
				$sSQL = "UPDATE users set notes='GOOGLE',fname='{$sName}',lname='{$userInfo['given_name']}' WHERE username='{$email}'";
				$ret = $db->Execute($sSQL);
                //$uret = true;				
              } 			  
		      ...
		      if (($uret) || ($ret = $db->Affected_Rows())) {
				...
				//set cookie
				if (paramload('SHELL','cookies')) {
					setcookie("cuser",$sUserName,time()+$this->time_of_session);//,time()+3600,"","panikidis.gr",0);
					setcookie("csess",session_id(),time()+$this->time_of_session);
				}		
			  }
            }
    ...
    function do_google_logout() {
      $UserName = GetGlobal('UserName');
	    
	  if ($this->is_google_logged_in()) {
