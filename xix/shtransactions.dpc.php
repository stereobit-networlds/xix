...
$__EVENTS['SHTRANSACTIONS_DPC'][7]='transviewcart';
$__EVENTS['SHTRANSACTIONS_DPC'][8]='transviewdoc';
$__EVENTS['SHTRANSACTIONS_DPC'][9]='transviewajax';

$__ACTIONS['SHTRANSACTIONS_DPC'][6]='transviewhtml';
$__ACTIONS['SHTRANSACTIONS_DPC'][7]='transviewcart';
$__ACTIONS['SHTRANSACTIONS_DPC'][8]='transviewdoc';
$__ACTIONS['SHTRANSACTIONS_DPC'][9]='transviewajax';
...
$__LOCALE['SHTRANSACTIONS_DPC'][3]='_PREVIEWCART;Document;Παραστατικό';	
$__LOCALE['SHTRANSACTIONS_DPC'][4]='_TRANSPORTCART;Transport;Διανομή';	
$__LOCALE['SHTRANSACTIONS_DPC'][5]='_CVIEWCART;View;Προβολή';
$__LOCALE['SHTRANSACTIONS_DPC'][6]='_loading;Loading;Περίμενε';
...
class shtransactions extends transactions {
    ...
    function event($event=null) {
       ...
       switch ($event) {
	     case 'transviewajax' : die($this->viewTransactions());
		                        break;		   
	     case 'transviewdoc'  : die($this->get_trdata(GetReq('id')));
		                        break;	   
	     case 'transviewcart' : die($this->details(GetReq('id')));
		                        break;
...
function viewTransactionCart(trid) {
  ...
  $.get('{$ajaxurl}transviewcart&id='+trid, function(data)
  {
...
function viewTransactionHtml(trid) {
  holder=document.getElementById('transview_'+trid)
  holder.style.height='600px';
  holder.style.width='100%';  
  
  $('#message_p_'+trid).html(''); 
  //iframe
  ifr = '<iframe src ="{$ajaxurl}transviewhtml&tid='+trid+'" width="100%" height="600px"><p>Your browser does not support iframes</p></iframe>';
  $('#transview_'+trid).html(ifr);
  
  /*$('#message_p_'+trid).html('<img src="images/loading.gif" alt="Loading"> {$Loading}').slideDown('fast');  
  
  $.get('{$ajaxurl}transviewhtml&tid='+trid, function(data)
  {
		$('#message_p_'+trid).html('');  
		$('#transview_'+trid).html(data);
  });*/
};

function viewTransactionDoc(trid) {
  holder=document.getElementById('transview_'+trid)
  holder.style.height='1px';
  holder.style.width='100%';
  ...
  $.get('{$ajaxurl}transviewdoc&id='+trid, function(data)
  {
		$('#message_p_'+trid).html(data);
		$('#transview_'+trid).html('');
  });  
...
JSTRANS;
	...
	protected function get_trdata($id=null) {
	    $db = GetGlobal('db');
	    $UserName = GetGlobal('UserName');	
		...
		$sSQL .= " SELECT date,code,name,qty,value FROM trdata WHERE";
		$sSQL .= " tid={$tid} AND cid='{$user}'";
		//echo $sSQL,'<br/><br/>';
		$result = $db->Execute($sSQL,2);

		if (!empty($result)) {
		  $ret = "<table id=\"trdata_table\"><tr><th>Date</th><th>Code-Name</th><th>Value-Qty</th></tr>";
   ...
   function save2disk($id,$data) {
	...
	function getTransactionsList() {
	   ...
	   if ($this->storetype=='DB') {  //db	
	     ...
	     if (!empty($res)) { 
           ...
           $browser = new browse($transtbl,localize('_TRANSLIST',getlocal()),$this->getpage($transtbl,$this->searchtext),null,null,null,false,'xixcp_div');
	       $out .= $browser->render("transviewajax",$ppager,$this,1,1,0,0);
	       unset ($browser);	
    ...
    function viewtrans($id,$fname,$lname,$status,$ddate,$dtime) {
       ...
       $label_cview = localize('_CVIEWCART',getlocal());
	   $cview_button ="&nbsp;<button class='btn' id='transviewcart_{$id}' onclick='viewTransactionCart(\"{$id}\")'>{$label_cview}</button>";
	   ...
	   $data[] = $cload_button . $cview_button;//$link;   
	   $attr[] = "left;10%";
	   ...
	   */
	   ...
	   if (is_readable($this->path . $id . ".html")) {	
		 ...
		 //js
         $label_preview = localize('_PREVIEWCART',getlocal());
		 $preview_button ="<button class='btn' id='transviewhtml_{$id}' onclick='viewTransactionHtml(\"{$id}\")'>{$label_preview}</button>";
		 		 
		 if (defined('TRANSPORT_DPC')) {
			$label_transport = localize('_TRANSPORTCART',getlocal());
			$transport_button = "&nbsp;<button class='btn' id='transviewtransport_{$id}' onclick='viewTransactionTransport(\"{$id}\")'>{$label_transport}</button>";		 
		 }
		 else {	//default trans data view 
			$label_transport = localize('_TRANSPORTCART',getlocal());
			$transport_button = "&nbsp;<button class='btn' id='transviewdoc_{$id}' onclick='viewTransactionDoc(\"{$id}\")'>{$label_transport}</button>";
		 }	
       }
	   else {
	     //$lnk = $lname;
	   ...
	   $data[] = $preview_button . $transport_button;//$lnk;   
	   $attr[] = "left;50%";   
	   ...
	   if (($mtemplate) && is_readable($mt)) {
	     ...
	     if (($_template) && is_readable($_t)) {
			...
			$tokens[] = "<p id='message_p_{$id}'></p>". 
			            "<div id='transview_{$id}'></div>";
