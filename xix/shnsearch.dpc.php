...
$__ACTIONS['SHNSEARCH_DPC'][4]='shnearch';
...
";
	...
	function search_union_attachments($terms=null) {
		$db = GetGlobal('db');	
		$lan = getlocal();	
	    ...
	    foreach ($appresult as $a=>$app) {	
			...
			$sSQL .= "(select code from {$appname}.pattachments";	
	...
	function search_attachments($terms=null) {
      ...
      $sSQL = $this->search_union_attachments($terms);
	  //echo $sSQL;	  
